...
import { Gas0Constants } from '@ui/src'
...
const useGetGameInfoChampion = (gameId: string | number | undefined) => {
  ...
  const { setTgName, tgNameList } = useGetTgName()
  const { setOwnerAddress, ownerList } = useGetOwnAddress()
  ...
  useEffect(() => {
    const users = roomInfo.players.map(v => v.user)
    if (IS_TELEGRAM) {
      setTgName(users)
    }
    if (Gas0Constants[chainId]) {
      setOwnerAddress(users)
    }
  }, [JSON.stringify(roomInfo.players), setOwnerAddress])
  useEffect(() => {
    ...
    return {
      roomInfo: {
        ...
        players: roomInfo.players.map((v, index) => {
          return {
            ...v,
            owner: (ownerList[v.user.toLowerCase()] ? ownerList[v.user.toLowerCase()] : v.owner ? v.owner : v.user) as Address,
            tgName: IS_TELEGRAM
  ...
  }, [JSON.stringify(roomInfo), JSON.stringify(ownerList), JSON.stringify(tgNameList), fetchGameInfo])
}

export default useGetGameInfoChampion
