...
const SubmitCardChampion: React.FC<ISubmitCard> = ({ disabled }) => {
  ...
  const handleSubmitCard = useCallback(async () => {
    ...
    try {
      ...
      const res = await lobbyContract.write.join([joinGame.signedCard], {
        account: account,
        maxFeePerGas: gasPrice[chainId],
        maxPriorityFeePerGas: gasPrice[chainId]
      })
  ...
  return (
    <div id="submit-card" style={{ width: '100%' }}>
      {isCard ? (
        ...
        <SubmitCardEle>
          <BoxWrap>
            <img decoding="async" loading="lazy" src={`${preStaticUrl}/img/bingo/lock.webp`} className={css.lockimg} />
          </BoxWrap>
          {isMobile ? (
...
export default SubmitCardChampion
