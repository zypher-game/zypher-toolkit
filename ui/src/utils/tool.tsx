...
export function pow10(num: number | string | BigNumber | undefined, decimals = 18): BigNumber {
  if (!num) {
...
export function bnPow10(num: number | string | BigNumber | undefined, decimals = 18): BigNumber {
  if (!num) {
    return new BigNumber(0)
  }
...
export function getShortenAddress(address: string | null | undefined, preLen = 6, endLen = 4): string {
  if (!address) {
    return ''
  }
...
export function formatCurrency(amount: number, precision = 2): string {
  const [unit, base] = Units.find(([, min]) => Number(amount) >= Number(min)) ?? ['', 1]
  return `${utils.commify((amount / (base as number)).toFixed(precision))}${unit}`
}
