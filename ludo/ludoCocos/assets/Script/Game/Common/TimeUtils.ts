...
class TimeUtil {
  ...
  public static getDayZero(timestamp: number, zone: number = 8): number {
    ...
    return (
      Math.floor((timestamp + zoneMills) / 86400000) * 86400000 - zoneMills
    );
  }
  ...
  public static getDayEnd(timestamp: number, zone: number = 8): number {
    let zoneMills: number = zone * JXDef.Time.HOUR;
    return (
      Math.floor((timestamp + zoneMills) / 86400000) * 86400000 -
      zoneMills +
      86400000 -
      1000
    );
  ...
  public static getDayHourStamp(
    ...
    return (
      TimeUtil.getMondayStamp(now) +
      (day - 1) * 86400000 +
      hour * JXDef.Time.HOUR +
      minute * JXDef.Time.MINUTE
    );
  }

  /** 获取给定时间对应当天的某个时间 */
  public static getDayTimeHM(
    time: number,
    hour: number,
    minute: number
  ): number {
    let dayTime: Date = new Date(time);
    let temp: number = new Date(
      dayTime.getFullYear(),
      dayTime.getMonth(),
      dayTime.getDate(),
      hour,
  ...
  /** 判断两个时间是否在同一月 */
  public static timeInSameMonth(
    time: number,
    now: number,
    preDeviation: number = 0
  ): number {
