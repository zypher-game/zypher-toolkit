...
export async function batchRequestFromGraph({ env }: { env: string }): Promise<(IGameList[] | undefined)[]> {
  const requests = supportedChainIds(env).map(async (chainIdLocal: ChainId) => {
    ...
    if (result.data && result.data.data && result.data.data.gameInfos) {
      if (result.data.data.gameInfos.length) {
        // 获取 idList
        const gameIdList = result.data.data.gameInfos.map((v: any) => parseInt(v.id, 16).toString())
        const lobbyAddrList = result.data.data.gameInfos.map((v: any) => v.lobbyAddr)
...
export const getRecentGameById = async ({
  ...
  try {
    ...
    if (multicall) {
      const { results } = await multicall.call([...paramsGameId, ...paramsCardId])
      if (results) {
        const map = new Map()
